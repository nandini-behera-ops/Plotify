const sampleListings = [
  {
    title:"Residential Plot", city:"Pune", area:1200, price:4500000,
    owner:"Ramesh", contact:"9876543210",
    desc:"Beautiful plot near main road",
    image:"https://via.placeholder.com/150x100?text=Residential", favorite:false
  },
  {
    title:"Farming Plot", city:"Mumbai", area:800, price:3500000,
    owner:"Sita", contact:"9876543211",
    desc:"Quiet area, good investment",
    image:"https://via.placeholder.com/150x100?text=Farming", favorite:true
  }
];

let userListings = JSON.parse(localStorage.getItem('listings')) || [];
let listings = [...sampleListings, ...userListings];
let editIndex=null;
...
function addOrUpdateListing(){
  ...
  if(!title||!city||!area||!price){ alert('Please fill required fields'); return; }
  ...
  // Save only user-added listings to localStorage
  const newUserListings = listings.filter(l => !sampleListings.includes(l));
  localStorage.setItem('listings', JSON.stringify(newUserListings));
...
function showFavorites(){ renderListings(listings.filter(l=>l.favorite)); }
...
renderListings();
  ...
  if (!loggedInUser) {
    // agar koi login nahi hai, login page pe bhej do
    window.location.href = "login.html";
  } else {
    // agar login hai, user ka naam show karo
